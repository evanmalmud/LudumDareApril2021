...
public class CharacterController2D : ObjectController2D {
    ...
    protected override void VerticalCollisions(ref Vector2 deltaMove)
    {
        if (OnLadder) {
            collisions.Reset();
            Vector2 origin = boxCollider.bounds.center + Vector3.up *
                (boxCollider.bounds.extents.y * Mathf.Sign(deltaMove.y) + deltaMove.y);
    ...
    public void Jump()
    {
        if (CanMove() && (!Dashing || cData.canJumpDuringDash)) {
            if (collisions.onGround || extraJumps > 0 || (cData.canWallJump && collisions.hHit)) {
                ...
                if (OnLadder) {
                    Vector2 origin = boxCollider.bounds.center + Vector3.up * boxCollider.bounds.extents.y;
                    Collider2D hit = Physics2D.OverlapCircle(origin, 0, collisionMask);
                    if (hit) {
                        return;
                    }
                    origin = boxCollider.bounds.center + Vector3.down * boxCollider.bounds.extents.y;
    ...
    public void ClimbLadder(float direction)
    {
        ...
        float radius = boxCollider.bounds.extents.x;
        Vector2 topOrigin = ((Vector2)boxCollider.bounds.center) + Vector2.up * (boxCollider.bounds.extents.y - radius);
        Vector2 bottomOrigin = ((Vector2)boxCollider.bounds.center) + Vector2.down *
            (boxCollider.bounds.extents.y + radius + skinWidth);
