...
public class ObjectController2D : RaycastController {
    // Colision parameters
    ...
    protected virtual void VerticalCollisions(ref Vector2 deltaMove) {
        ...
        for (int i = 0; i < verticalRayCount; i++) {
            ...
            if (hit) {
                //PlatformController platform = hit.transform.gameObject.GetComponent<PlatformController>();
                //if (platform) {
                //    Debug.Log("On Platform vert coll");
                //    deltaMove.y = ((hit.distance + (platform.speed.y * Time.deltaTime)) - skinWidth) * directionY;
                //    rayLength = (hit.distance + (platform.speed.y * Time.deltaTime));
                //} else {
